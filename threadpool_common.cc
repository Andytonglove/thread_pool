...
static MYSQL_SYSVAR_UINT(idle_timeout, threadpool_idle_timeout, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_UINT(oversubscribe, threadpool_oversubscribe, PLUGIN_VAR_RQCMDARG, "How many additional active worker threads in a group are allowed.", NULL, NULL, 3, 1, 1000, 1);

static MYSQL_SYSVAR_UINT(toobusy, threadpool_toobusy, PLUGIN_VAR_RQCMDARG, "How many additional active and waiting worker threads in a group are allowed.", NULL, NULL, 13, 1, 1000, 1);

static MYSQL_SYSVAR_UINT(size, threadpool_size, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_UINT(stall_limit, threadpool_stall_limit, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_UINT(max_threads, threadpool_max_threads, PLUGIN_VAR_RQCMDARG, "Maximum allowed number of worker threads in the thread pool", NULL, NULL, MAX_CONNECTIONS, 1, MAX_CONNECTIONS, 1);
...
static MYSQL_THDVAR_ENUM(high_prio_mode,
                         ...
                         &threadpool_high_prio_mode_typelib);
...
static int groups_fill_table(THD* thd, TABLE_LIST* tables, Item*) {
    ...
    for (uint i = 0; i < MAX_THREAD_GROUPS && all_groups[i].pollfd != -1; i++) {
        ...
        /* QUEUE LENGTH */
        uint queue_len = group->high_prio_queue.elements() + group->queue.elements();
